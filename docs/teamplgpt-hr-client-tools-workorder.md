...
TeamplGPT에 HR agent skill 2종(hr-attendance, hr-personnel)이 배포됨. 이 스킬들은
...
### 1.5 브리지 endpoint allowlist (이 18개 외 실행 거부)
...
- SAL* 3경로 + CommonCode.do는 hr-salary — 급여 금액 경유하므로 응답 body 로그 금지(§2.2) 적용.
...
handler(event):
  ...
  3) spec.path가 §1.5 allowlist에 없으면 거부 회신
  4) form 값 검사: searchType=mobile 거부 / "$SELF_STAFF_ID" → §1.4 규칙으로 치환
...
- `hrBase`: `window.erxesSettings.messenger.hrBaseUrl` (main.jsp 1368행에서 이미 전달됨).
  **⚠️ 검증 항목**: 이 값이 컨텍스트 경로(`/kiwibox`)를 포함하는지 실측 — 미포함이면
  `hrBase + "/kiwibox" + path`로 보정. 값이 비면 `location.origin` 폴백.
...
- embed 인스턴스 생성: 대상 워크스페이스(5240) 연결, `allow_tool_calling=true`,
  `client_tool_execution=true`, allowed skills = hr-attendance·hr-personnel,
  `allowlist_domains`에 위젯 서버 도메인 등록 → `TEAMPLGPT_EMBED_ID` 확보.
...
- [ ] hrBaseUrl 컨텍스트 경로 포함 여부 실측 결과를 이 문서에 추기
