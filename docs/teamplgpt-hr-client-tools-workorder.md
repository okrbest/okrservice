...
TeamplGPT에 HR agent skill 4종(hr-attendance, hr-personnel, hr-salary, hr-approval)이 배포됨. 이 스킬들은
...
### 1.5 브리지 endpoint allowlist (이 19개 외 실행 거부)
...
- hr-personnel: getMBLPrtEmpCard(Pop) + getMBLHrBassiem(Org/Member)List + getTodoIconCnt + getScheduleDay + getContactList
- hr-salary: SAL* 3 + CommonCode.do
- hr-approval: EAPRequestMgr.do

- SAL* 3경로 + CommonCode.do(hr-salary) + EAPRequestMgr.do(hr-approval)는 급여·결재 데이터
  경유 — 응답 body 로그 금지(§2.2) 적용.
...
- allowed skills:
  - 기본: hr-attendance, hr-personnel (저위험, self)
  - hr-salary: **급여 금액 노출 — 고객사 정책 승인 후** 추가 (2단계 조회, 계좌 항목 제외됨)
  - hr-approval: 결재함 목록만(본문 제외, self) — 정책 확인 후 추가
