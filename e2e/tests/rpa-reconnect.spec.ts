...
test.beforeEach(async () => {
  ...
  try {
    await conn.db!.collection('rpa_messages').deleteMany({ loginId: LOGIN_ID });
  } finally {
    await conn.close();
  }
});
...
async function openWidgetPage(page: Page) {
  ...
  await page.frameLocator('#erxes-launcher').locator('.erxes-launcher').waitFor({ timeout: 30_000 });
...
test('위젯 재접속 후 누락 없이 메시지를 수신한다 (WebSocket 재연결 시나리오)', async ({
  ...
  await frame.getByText('Chatbot').waitFor({ timeout: 30_000 });
  await frame.getByText('Chatbot').click();
  // 챗봇 뷰 로드 확인 (인사 메시지)
  await frame.getByText('안녕하세요').waitFor({ timeout: 15_000 });
  ...
  await frame2.getByText('Chatbot').waitFor({ timeout: 30_000 });
  await frame2.getByText('Chatbot').click();
  // ChatbotView 렌더 완료 대기 (인사 메시지가 보여야 loadHistory도 완료 근처)
  await frame2.getByText('안녕하세요').waitFor({ timeout: 15_000 });
