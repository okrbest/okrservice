...
const handleLauncherIframeLoad = async () => {
  ...
  if (launcherIframeDocument) {
    ...
    const style = launcherIframeDocument.createElement("style");
    style.textContent = window.erxesSettings?.messenger?.css || "";
    launcherIframeDocument.head.appendChild(style);
...
window.addEventListener("message", async (event: MessageEvent) => {
  ...
  if (data.fromErxes && data.connectionInfo) {
    ...
    const { uiOptions, dealData } = widgetsMessengerConnect || {};
    isDealMode =
    ...
    // Deal 모드: 플로팅 버튼 숨김 (위젯 열릴 때만 닫기 버튼으로 표시)
    const displayValue =
      showLauncherSetting === false || isDealMode ? "none" : "flex";
  ...
  if (data.fromErxes && data.source === "fromMessenger") {
    ...
    if (message === "messenger") {
      ...
      if (isVisible) {
        ...
        } else if (showLauncherSetting === false || isDealMode) {
        ...
        launcher.innerHTML = ``;
        if (isMobile) {
          if (showLauncherSetting === false || isDealMode) {
            launcherIframe.style.display = "none";
            launcher.style.display = "none";
          } else {
            launcherIframe.style.display = "block";
