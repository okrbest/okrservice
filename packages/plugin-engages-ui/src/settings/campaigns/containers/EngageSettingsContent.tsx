...
const SettingsContainer = () => {
  const {
    loading: loadingConfigDetail,
    data: configDetailData,
    refetch: refetchConfigDetail,
  } = useQuery<EngageConfigQueryResponse>(
    gql(engageQueries.engagesConfigDetail)
  );

  const {
    loading: loadingVerifiedEmails,
    data: verifiedEmailsData,
    refetch: refetchVerifiedEmails,
  } = useQuery<EngageVerifiedEmailsQueryResponse>(
    gql(engageQueries.verifiedEmails)
  );

  const { loading: loadingUsers, data: usersData } = useQuery(
    gql(engageQueries.verifiedUsers)
  );

  const [verifyEmailMutation] = useMutation(gql(engageMutations.verifyEmail));
  const [removeVerifiedEmailMutation] = useMutation(
    gql(engageMutations.removeVerifiedEmail)
  );
  const [sendTestEmailMutation] = useMutation(
    gql(engageMutations.sendTestEmail)
  );
