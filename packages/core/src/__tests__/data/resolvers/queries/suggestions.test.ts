...
describe('chatbotSuggestions', () => {
  ...
  it('2글자 미만이면 빈 배열 반환', () => {
    const result = resolve(null, { keyword: '출' });
    expect(result).toEqual([]);
  });

  it('prefix 매칭으로 여러 결과 반환', () => {
    const result = resolve(null, { keyword: '출근' });
  ...
  it('"출" prefix → 출근, 출퇴근변경, 출장 모두 포함', () => {
    const result = resolve(null, { keyword: '출' });
    expect(result).toEqual([]);

    const result2 = resolve(null, { keyword: '출근' });
    const keywords = result2.map((r: any) => r.keyword);
    expect(keywords).toContain('출근');
  });

  it('매칭 없으면 빈 배열', () => {
    const result = resolve(null, { keyword: '없는키워드abc' });
    expect(result).toEqual([]);
  });

  it('chatbotId 파라미터를 무시해도 정상 동작', () => {
    const result = resolve(null, { keyword: '휴가', chatbotId: 'test-corp' });
    expect(result.length).toBeGreaterThan(0);
